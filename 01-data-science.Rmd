...
Science can be replicated. You should also be able to reproduce an empirical finding, which implies we also think about science as a procedure.
...
In the most recent presidential election, the American National Election Study asked questions about presidential vote and about household income in 2024. Household income is not a perfect proxy or measure of personal finances, but it is probably a good indicator of what we are interested in – are you rich or poor? The sample is divided into three groups - the lowest 20%, the top 20% and the middle 60%.  We can use a simple table to compare the presidential vote of the rich and poor. What percentage of lower income households (annual income of less than $25,000) voted for the Donald Trump? What percentage of rich households (annual income exceeding $150,000) voted for Donald Trump? 
...
# Should be the "full sample post-election weight" - this is fresh respondents, no panel
# "POST-ELECTION RAKED WEIGHT: FACE-TO-FACE (FTF) & WEB SAMPLES COMBINED W/PAPI [PRELIMINARY]"
anes2024$weight<-anes2024$V240105b

# recode and identify missing values for vote and income
anes2024$republican<-car::recode(anes2024$V242067, "-9:-1=NA; 1=0; 2=1; 3:6=NA")
anes2024$income3<-car::recode(anes2024$V241566x, "-9:0=NA; 1:8=1; 9:25=2; 26:28=3")

#anes2024$income2<-car::recode(anes2024$V201617x, "-9:0=NA; 1:3=1; 4:19=NA; 20:22=2")
# Not sure how to recode this to match up with earlier - made it 1/3s

anes2024_table <- anes2024 %>% filter(!is.na(weight), !is.na(income3), !is.na(republican))



# Calculate proportion of the high and low income who vote for Trump over all other presidential candidates

table11<-anes2024_table %>% group_by(income3) %>% summarise(100*weighted.mean(republican, weight))

table11$income3 <- factor(table11$income3,
levels = c(1,2,3),
...
Why do you need to understand this? Regression is widely used in a variety of contexts – from program evaluation to marketing to campaigns. If you work in an organization that routinely collects and analyzes data, someone in the organization will be using statistical models to make sense of that data. After you complete this course, you will have a better understanding of what they are doing and how they do it.
...
This document and all of the output that you create in this course is generated using R and RMarkdown. If you want to land a job doing data science work, you will need to take courses that teach you how to use some or several types of statistical software.  This course introduces you to R and a cloud-based front-end for R, RStudio (hosted by Posit).  

## What’s next?

The next chapter introduces you to some basic numbers we use to describe the distribution of variables in a dataset. The substantive focus is how people identify with the major political parties. What is party identification? Who identifies with the Republican Party? Who identifies with the Democratic Party? Who are the Independents? We will use the 2024 American National Election Study to get a handle on these questions and learn how to interpret statistical output to convey what we know.
...
- What does the observed $\beta_1$ tell us about the relationship between $X$ and $Y$ in the sample?
...
A simple scatter plot of this data for the 50 U.S. states is reproduced as Figure 6.1, below.

**Figure \@ref(fig:figure61)  Scatterplot of women in the state legislature as a function of the proportion of state residents who are Christian**
...
```{r anes, echo=FALSE}
# Read the latest version of the ANES (April 30, 2025)
...
# Income
anes2024$income<-car::recode(anes2024$V241566x, "-9:0=NA")
anes2024$income3<-car::recode(anes2024$V241566x, "-9:0=NA; 1:8=1; 9:25=2; 26:28=3")
anes2024$income3 <- factor(anes2024$income3,
levels = c(1,2,3),
labels = c("Under $25,000", "$25,000 to $150,000", "Over $150,000"))

# Education
anes2024$education<-car::recode(anes2024$V241463, "-9:0=NA; 95=NA")
anes2024$college  <-car::recode(anes2024$V241463, "-9:0=NA; 95=0; 1:12=0; 13:16=1")

# Feeling thermometer - Donald Trump
anes2024$ft_Trump<-car::recode(anes2024$V241157, "-9:-1=NA")

# Ideology self-placement
anes2024$ideology<-car::recode(anes2024$V241177, "-9:-7=NA; 99=NA") 
anes2024$ideology <- factor(anes2024$ideology,
levels = c(1,2,3,4,5,6,7), ordered=TRUE,
...
# Race in 4 categories
anes2024$race4<-car::recode(anes2024$V241501x , "-9:-3=NA; 1=1; 2=2; 3=3; 4:6=4")
...
**Figure \@ref(fig:figure64)  Donald Trump feeling thermometer, as a function of party identification**
```{r figure64, echo=FALSE, fig.cap="\\label{fig:figure64}", out.width = '90%', fig.align = 'center'}
figure64<- anes2024 %>% filter(anes2024$partyid!="NA") %>% group_by(partyid) %>% summarise(mean=mean(ft_Trump, na.rm=TRUE))
...
model1<-lm(ft_Trump~as.numeric(partyid), data=anes2024)
library(stargazer)
...
For every unit increase in party id from 0->1, or from 1->2, the feeling thermometer increases by 13.9 points.  

So the difference between strong Democrats (1) and strong Republicans (7) is equal to (7-1)*13.9 or 83 points!  Even larger than we expected.
...
**Table \@ref(tab:model2) Party identification as a function of income.**
<span style="color: white;">(\#tab:model2)</span> 
```{r model2, echo=FALSE, warning=FALSE, message=FALSE, comment=""}
model2<-lm(as.numeric(partyid)~income, data=anes2024)
...
**Table \@ref(tab:model3) Donald Trump feeling thermometer and gender.**
<span style="color: white;">(\#tab:model3)</span> 
```{r model3, echo=FALSE, warning=FALSE, message=FALSE, comment=""}

model3<-lm(ft_Trump~as.numeric(female), data=anes2024)
...
For women, $Y$ = 43.0 + -6.4*1 = 36.6

For men,   $Y$ = 43.0 + -6.4*0 = 43.0

While the effect or link is statistically significant, so men are slightly warmer towards Trump, the effect is small (just over 6 points).
...
**Table \@ref(tab:model4)  Donald Trump feeling thermometer and race.**
<span style="color: white;">(\#tab:model4)</span> 
```{r model4, echo=FALSE, warning=FALSE, message=FALSE, comment=""}

model3<-lm(ft_Trump~as.numeric(minority), data=anes2024)
...
The regression output tells us that, for White respondents, $Y$ = 42.7 + -10.6*0 = 42.7.  For Minorities, $Y$ =  42.7-10.6 = 32.1.  So there is large difference between White and minority voters (just over 10 points), but even White voters are –as a group- only are only lukewarm towards Donald Trump.

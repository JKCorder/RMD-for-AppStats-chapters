...
# Pay attention to politics and elections
anes2024$interest<-car::recode(anes2024$V241004, "-9:-5=NA; 1=5; 2=4; 3=3; 4=2; 5=1")
anes2024$interest <- factor(anes2024$interest,
...
figure2<-anes2024 %>% filter(anes2024$partyid!="NA")
...
# anes2024$female<-car::recode(anes2024$V201600, "1=0; 2=1; -9=NA") # 2 is Female, recoded to 1 is female
# anes2024$female <- factor(anes2020$female,
#     levels = c(1,0),
#     labels = c("Women", "Men"))
# 
# figure_test<-anes2020 %>% filter(anes2020$partyid!="NA", anes2020$female!="NA")
# ggplot (figure_test, aes(x=female, y=ft_Trump))+facet_wrap(figure_test$partyid, ncol=7)+geom_bar(stat = "summary", fun = mean, fill="lightblue", color="black") +ylab("Feeling thermometer") + xlab("") + theme(strip.text.x = element_text(size = 5)) + theme(axis.text.x=element_text(size=rel(1.0)))
...
figure5<-anes2024 %>% group_by(income) %>% summarise(mean=mean(as.numeric(partyid), na.rm=TRUE))
...
```{r example, echo=FALSE, comment="", message=FALSE, warning=FALSE }
...
figure7<-anes2024 %>% group_by(partyid) %>% summarise(mean=mean(as.numeric(interest), na.rm=TRUE))
...
Notice, again, that leaners are more like strong partisans - very interested in politics compared to weak partisans.  The interest scale goes from 1 to 5 and 3.0 indicates that someone follows politics "About half of the time."  The blue span is the best-fitting curve.
...
df<- anes2024 %>% select(ideology, partyid, income, education, weight) %>% drop_na() %>% mutate(ideology=as.numeric(ideology), partyid=as.numeric(partyid))
test<-cov.wt(df, wt=df$weight, cor=TRUE)
round(test$cor[1:4,1:4], digits=3)
# 
```

This output is in the form of a correlation matrix – so you can see all three correlations – party vs. ideology, education vs. ideology, and income vs. ideology in one table.  In this sample, the correlation between ideology and income is close to zero (-0.078) – there is a weak relationship.  The link between education and ideology is a stronger and negative (-0.202) – more education means a slightly lower ideology or more liberal.  It would be safe to conclude that poorly educated respondents are the most conservative and highly educated are the most liberal. But party id is clearly the best predictor (0.737 - the farthest of any of the correlations from zero). We can also see that education and income are more closely  related to each other (0.429) than either are to ideology.  The number in this table is slightly different than immediately above since a few observations were missing - meaning that a few respondents did not answer the ideology question.
